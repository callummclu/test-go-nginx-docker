...
export async function removePost(id: string, token: string, username: string) {
  return fetch(
    `${process.env.REACT_APP_BACKEND_URI}/post/${username}/${id}/${token}`,
    {
      method: "GET",
    }
  )
...
export function getPosts(
  ...
  let uri =
    searchParameters && searchParameters.length > 0
      ? `${process.env.REACT_APP_BACKEND_URI}/post?searchParams=${searchParameters}&itemsPerPage=${itemsPerPage}&page=${page}`
      : `${process.env.REACT_APP_BACKEND_URI}/post`;
  return fetch(uri);
}
...
export function toggleLikedPost(id: string, token: string, username: string) {
  return fetch(
    `${process.env.REACT_APP_BACKEND_URI}/post/like/${id}/${username}/${token}`,
    {
      method: "POST",
    }
  );
